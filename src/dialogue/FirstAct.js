...
const FirstAct = () => {
  ...
  const renderIntro = () => {
    ...
    return (
      <p>
        {dialogue.introDialogue[dialogueCount]}
  ...
  const renderFirstResponse = () => {
    return <p>{dialogue.npcDialogueFirst[npcReply]}</p>;
  };
  ...
  const generateUserResponses = () => {
    ...
    return (
      <p>
        {test.map((a, b) => {
          return (
            <button value={b} onClick={(e) => userDialogueHandler(e)}>
              {a}
            </button>
          );
        })}
